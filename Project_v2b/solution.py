...
class BayesianLayer(torch.nn.Module):
    ...
    def forward(self, inputs): #logsigma ?
        ...
        self.log_prior = self.log_prob(self.samples_gaussian, self.prior_mu, self.prior_sigma) + \
                         self.log_prob(self.samples_gaussian_bias, self.prior_bias_mu, self.prior_bias_sigma)
        self.log_variational_posterior = self.log_prob(self.samples_gaussian, self.weight_mu, torch.exp(self.weight_logsigma)) + \
                                         self.log_prob(self.samples_gaussian_bias, self.bias_mu, torch.exp(self.bias_logsigma))
...
class BayesNet(torch.nn.Module):
    ...
    def predict_class_probs(self, x, num_forward_passes=11):
        ...
        for i in range(num_forward_passes):
            prob = F.softmax(self.forward(x), dim=1) #each output (batch_size, 10) size
            probs.append(prob)
