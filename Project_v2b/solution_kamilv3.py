...
class BayesianLayer(torch.nn.Module):
    ...
    def forward(self, inputs): #logsigma ?
        ...
        self.log_prior = self.log_prob(self.samples_gaussian, self.prior_mu, self.sigma(self.prior_sigma)) / self.samples_gaussian.shape[1]
        self.log_variational_posterior = self.log_prob(self.samples_gaussian, self.weight_mu, self.weight_sigma)+ self.log_prob(self.samples_gaussian_bias, self.bias_mu, self.bias_sigma) / self.samples_gaussian.shape[1]
